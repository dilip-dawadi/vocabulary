...
export async function getAllWords(page, pageSize = 5) {
...
export async function getUnunderstoodWords(page, pageSize = 5) {
    const [words] = await Promise.all([
        db
            .select()
            .from(wordsTable)
...
export async function getUnderstoodWords(page, pageSize = 5) {
    const [words] = await Promise.all([
        db
            .select({
...
export async function getWordsForLast24Hours(page = 1, pageSize = 5) {
    return db
        .select({
        id: wordsTable.id,
        ...
        .from(wordsTable)
        .where(between(wordsTable.created_at, sql `now() - interval '1 day'`, sql `now()`)) // Filter for the last 24 hours
